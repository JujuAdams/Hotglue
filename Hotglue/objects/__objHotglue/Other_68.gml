...
if ((__server != undefined) && (async_load[? "port"] == __HotglueSystem().__localhostPort))
{
    ...
    else
    {
        if (async_load[? "server"] == __server)
        {
            ...
            if ((_codePos > 0) && (_httpPos > 0) && (_httpPos > _codePos))
            {
                ...
                var _request = new __HotglueClassHttpRequest($"https://github.com/login/oauth/access_token?client_id={HOUTGLUE_GITHUB_CLIENT_ID}&client_secret={HOUTGLUE_GITHUB_CLIENT_SECRET}&code={_code}", "POST");
                ...
                _request.Callback(function(_request, _success, _result)
                {
                    if (not _success)
                    {
                        ...
                        if (is_callable(__gitHubAuthCallback))
                        {
                            __gitHubAuthCallback(false);
                        }
                    }
                    else
                    {
                        ...
                        with(__HotglueSystem())
                        {
                            __githubUserAuthToken = _json.access_token;
                            
                            if (is_callable(__gitHubAuthCallback))
                            {
                                __gitHubAuthCallback(true);
                            }
                        }
                    }
                });
